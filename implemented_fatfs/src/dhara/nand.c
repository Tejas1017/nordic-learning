...
void dhara_nand_mark_bad(const struct dhara_nand *n, dhara_block_t b)
{
    ...
    row_address_t row = {.block = b, .page = 0};
    // call spi_nand layer
    MT29F4G_flash_flashblock_mark_bad(row); // ignore ret
}
...
int dhara_nand_prog(const struct dhara_nand *n, dhara_page_t p,  uint8_t *data,
                    dhara_error_t *err)
{
    // construct row address -- dhara's page address is identical to an MT29F row address
    row_address_t row = {.whole = p};
    // call spi_nand layer
    int ret = MT29F4G_flash_write_page(row, 0, data, SPI_NAND_PAGE_SIZE);
    if (SPI_NAND_RET_OK == ret) { // success
        return 0;
    }
...
int dhara_nand_is_free(const struct dhara_nand *n, dhara_page_t p)
{
    ...
    // call spi_nand layer
    bool is_free;
    int ret = MT29F4G_flash_flashpage_is_free(row, &is_free);
    if (SPI_NAND_RET_OK != ret) {
                    ...
                    uint8_t *data, dhara_error_t *err)
{
    ...
    row_address_t row = {.whole = p};
    // call spi_nand layer
    int ret = MT29F4G_flash_page_read(row, offset, data, length);
    if (SPI_NAND_RET_OK == ret) { // success
        return 0;
    }
                    ...
                    dhara_error_t *err)
{
    ...
    // call spi_nand layer
    int ret = MT29F4G_flash_page_copy(source, dest);
    if (SPI_NAND_RET_OK == ret) { // success
        return 0;
    }
